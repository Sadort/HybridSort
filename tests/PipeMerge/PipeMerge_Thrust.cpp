...
int main(void)
{
    ...
    ThrustSort(h_key_array, d_key_array, h_value_array, d_value_array, number_of_elements, batch_size, pinned_M_size, nthreads);
    ...
    for (int i = 0; i < number_of_batches / 2; i++)
    {
        if (i == (number_of_batches / 2) - 1) {
            batches.push_back(std::make_pair(&indices[2*i*batch_size], &indices[2*i*batch_size+batch_size]));
            batches.push_back(std::make_pair(&indices[2*i*batch_size+batch_size], &indices[number_of_elements]));
            break;
        }
        batches.push_back(std::make_pair(&indices[2*i*batch_size], &indices[2*(i+1)*batch_size]));
    }
