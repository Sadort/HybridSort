...
void ThrustSort(uint64_t *h_key_array, uint64_t *d_key_array[2], uint64_t *h_value_array, uint64_t *d_value_array[2], uint64_t number_of_elements, uint64_t batch_size, uint64_t pinned_M_size, int nthreads)
{
        ...
        for (int s = 0; s < 2; s++) {
            if (i == 0 && s == 0) {
                for (int b = 0; b < number_of_buffers; b++) {
                    ParMemcpy(pinned_key_M[0], &h_key_array[start_index_s0+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
                ...
                for (int b = 0; b < number_of_buffers; b++) {
                    ParMemcpy(pinned_value_M[0], &h_value_array[start_index_s0+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
            ...
            if (s == 0) {
                ...
                for (int b = 0; b < number_of_buffers; b++) {
                    ParMemcpy(pinned_key_M[1], &h_key_array[start_index_s1+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
                ...
                for (int b = 0; b < number_of_buffers; b++) {
                    ParMemcpy(pinned_value_M[1], &h_value_array[start_index_s1+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
                ...
                for (int b = 0; b < number_of_buffers; b++) {
                    ...
                    ParMemcpy(&h_key_array[start_index_s0+b*(pinned_M_size/2)], pinned_key_M[0], pinned_M_size/2, mem_threads);
            ...
            if (s == 1 && i != (number_of_batches / 2) - 1) {
                for (int b = 0; b < number_of_buffers; b++) {
                    cudaMemcpyAsync(pinned_key_M[1],
                                    ...
                                    cudaMemcpyDeviceToHost,
                                    streams[1]);
                    ParMemcpy(pinned_key_M[0], &h_key_array[start_index_s2+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
                    cudaStreamSynchronize(streams[1]);
                    ...
                    cudaMemcpyAsync(&d_key_array[0][b*(pinned_M_size/2)],
                                    ...
                                    cudaMemcpyHostToDevice,
                                    streams[0]);
                    ParMemcpy(&h_key_array[start_index_s1+b*(pinned_M_size/2)], pinned_key_M[1], pinned_M_size/2, mem_threads);
                    cudaStreamSynchronize(streams[0]);
                }
                for (int b = 0; b < number_of_buffers; b++) {
                    cudaMemcpyAsync(pinned_value_M[1],
                                    ...
                                    cudaMemcpyDeviceToHost,
                                    streams[1]);
                    ParMemcpy(pinned_value_M[0], &h_value_array[start_index_s2+b*(pinned_M_size/2)], pinned_M_size/2, mem_threads);
                    cudaStreamSynchronize(streams[1]);
                    ...
                    cudaMemcpyAsync(&d_value_array[0][b*(pinned_M_size/2)],
                                    ...
                                    cudaMemcpyHostToDevice,
                                    streams[0]);
                    ParMemcpy(&h_value_array[start_index_s1+b*(pinned_M_size/2)], pinned_value_M[1], pinned_M_size/2, mem_threads);
                    cudaStreamSynchronize(streams[0]);
                }
            }
