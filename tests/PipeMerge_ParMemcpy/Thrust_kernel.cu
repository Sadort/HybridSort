...
void ThrustSort(ulong2 *h_key_array, ulong2 *d_key_array[2], uint64_t number_of_elements, uint64_t batch_size, uint64_t pinned_M_size, int nthreads)
{
    ...
    cudaHostAlloc( (void**)&pinned_M[0], (pinned_M_size / 2) * sizeof(ulong2), cudaHostAllocDefault );
    cudaHostAlloc( (void**)&pinned_M[1], (pinned_M_size / 2) * sizeof(ulong2), cudaHostAllocDefault );
            ...
            if (i == 0 && s == 0) {
                ...
                for (int b = 0; b < number_of_buffers; b++) {
                    ...
                    cudaMemcpyAsync(&d_key_array[0][b*(pinned_M_size/2)],
                                    ...
                                    cudaMemcpyHostToDevice,
                                    streams[0]);
                    ParMemcpy(&h_key_array[start_index_s2+b*(pinned_M_size/2)], pinned_M[1], pinned_M_size/2, mem_threads);
                    cudaStreamSynchronize(streams[0]);
                }
                thrust::sort(thrust::cuda::par(alloc).on(streams[0]), th_key_array[0], th_key_array[0]+batch_size);
