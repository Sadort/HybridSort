...
void ThrustSort(uint64_t *h_key_array, uint64_t *d_key_array[2], uint64_t *h_value_array, uint64_t *d_value_array[2], uint64_t number_of_elements, uint64_t batch_size, int nthreads)
{
    ...
    for (int i = 0; i < number_of_batches / 2; i++) {
        for (int s = 0; s < 2; s++) {
            if (i == 0 && s == 0) {
                ParMemcpy(pinned_key_M[0], &h_key_array[start_index_s0], batch_size, mem_threads);
                ParMemcpy(pinned_value_M[0], &h_value_array[start_index_s0], batch_size, mem_threads);
            ...
            if (s == 0)
            {
                ParMemcpy(pinned_key_M[1], &h_key_array[start_index_s1], batch_size, mem_threads);
                ParMemcpy(pinned_value_M[1], &h_value_array[start_index_s1], batch_size, mem_threads);
            ...
            else if (s == 1)
            {
                ...
                ParMemcpy(&h_key_array[start_index_s0], pinned_key_M[0], batch_size, mem_threads);
                ParMemcpy(&h_value_array[start_index_s0], pinned_value_M[0], batch_size, mem_threads);

            }
            if (s == 1 && i != (number_of_batches / 2) - 1) {
                ParMemcpy(pinned_key_M[0], &h_key_array[start_index_s2], batch_size, mem_threads);
                ParMemcpy(pinned_value_M[0], &h_value_array[start_index_s2], batch_size, mem_threads);
                ...
                ParMemcpy(&h_key_array[start_index_s1], pinned_key_M[1], batch_size, mem_threads);
                ParMemcpy(&h_value_array[start_index_s1], pinned_value_M[1], batch_size, mem_threads);
            }
